...
for pth in ['../rp_lidar_out_clockwise.bag','../rp_lidar_out_counterclockwise.bag', 
        '../rp_lidar_out_c_bound.bag','../rp_lidar_out_ac_bound.bag']:
...
overall_df = pd.DataFrame({'Servo': overall_servo_samples, 'Speed': overall_speed_samples})
lidar_df = pd.DataFrame(overall_lidar_samples)

# Define file names for your CSV files
overall_csv_filename = 'speed_servo_data.csv'
lidar_csv_filename = 'lidar_data.csv'

# Save the dataframes to CSV files
overall_df.to_csv(overall_csv_filename, index=False)
lidar_df.to_csv(lidar_csv_filename, index=False)

print(f'Data saved to {overall_csv_filename}, and {lidar_csv_filename}.')
...
#Recording predicted values
y_pred = model.predict(overall_lidar_samples)

# Extract speed and servo predictions
speed_predictions = y_pred[:, 1]  # Index 1 for speed
servo_predictions = y_pred[:, 0]  # Index 0 for servo



# Create a DataFrame for predicted data
pred_df = pd.DataFrame({
    'Predicted Servo': servo_predictions,
    'Predicted Speed': speed_predictions
})
# Define the file name for the predictions CSV
pred_csv_filename = 'pred_sevo_speed.csv'

# Save the predicted data to a CSV file
pred_df.to_csv(pred_csv_filename, index=False)

print(f'Predicted data saved to {pred_csv_filename}.')
...
tflite_model_path = model_file+".tflite"
...
quantized_tflite_model = converter.convert()
tflite_model_path = model_file+"float16.tflite"
with open(tflite_model_path, 'wb') as f:
    f.write(quantized_tflite_model)
    print (tflite_model_path+" is saved. copy this file to the robot")
print('Tf_lite Model also saved')
output_header_file = tflite_model_path[:-7]+'.cc'
subprocess.run(["xxd", "-i", tflite_model_path], stdout=open(output_header_file, "w"))
print("C header file generated successfully:", output_header_file)
...
quantized_tflite_model = converter.convert()
tflite_model_path = model_file+"_micro.tflite"
with open(model_file+"_micro.tflite", 'wb') as f:
    f.write(quantized_tflite_model)
    print (model_file+"_micro.tflite is saved. copy this file to the robot")
...
print("==========================================")
model_name = './rp_f1_tenth_model'
interpreter = tf.lite.Interpreter(model_path=model_name+'.tflite')
interpreter_micro = tf.lite.Interpreter(model_path=model_name+'_micro.tflite')
